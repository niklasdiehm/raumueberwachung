...
current_date = datetime.now(timezone.utc).replace(tzinfo=timezone.utc)
...
while (date_seven_days_before < current_date):
    ...
    request = requests.get("https://fa-roomitor.azurewebsites.net/api/measurements?code=VOXG7U915GhnWo1dRCd-wuNrhgM897uDp4Lae4-kTKZCAzFu7O0DIA==" +
                           "&timestamp[after]=" +
